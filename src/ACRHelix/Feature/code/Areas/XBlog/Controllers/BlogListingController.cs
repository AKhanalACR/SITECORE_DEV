...
namespace Sitecore.Feature.XBlog.Areas.XBlog.Controllers
{
    public class BlogListingController : Controller
    {
        ...
        public ActionResult BlogListing()
        {
            ...
            Log.Error("Log in Controller 001:" + Request.Url.PathAndQuery, this);
            Log.Error("Log in Controller 002:" + XBSettings.XBCategoryUrlPattern, this);
            ...
            {
                Log.Error("Log in Controller 1:"+ urlCategoryName, this);
                Category categoryItem = CategoryManager.GetCategoryByName(Context.Item, urlCategoryName);
                if (categoryItem != null)
                {
                    Log.Error("Log in Controller 2:" + categoryItem.InnerItem.ID.ToString(), this);
            ...
            Log.Error("Log in Controller 3:" + categoryID, this);

            startRowIndex = (currentPage - 1) * maximumRows;
            model.CurrentPage = currentPage;
      Data.ID[] idFeaturedBLog = { new Data.ID("{11111111-1111-1111-1111-111111111111}") };// new Data.ID[1];
     // idFeaturedBLog[0] = new Data.ID("{11111111-1111-1111-1111-111111111111}");
      model.BlogPosts = BlogManager.GetBlogPosts(Sitecore.Context.Item, categoryID, authorID, tagID, searchText, startRowIndex, maximumRows, idFeaturedBLog);
