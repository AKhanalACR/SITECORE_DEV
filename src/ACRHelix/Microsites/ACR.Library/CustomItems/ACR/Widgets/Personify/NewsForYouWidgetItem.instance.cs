...
{
public partial class NewsForYouWidgetItem 
{
	...
	private List<Item> PerformSearch(SafeDictionary<string> filters)
	{
		...
		foreach (NewsArticleItem p in index.GetNewsArticles(filters, null, NewsIndex.GetDefaultSort(4)))
		{
			Item i = p;
			newsItems.Add(i);
		}
	...
	private void FetchRelatedNews()
	{
		...
		{
			_relatedNews = PerformSearch(acrProfile.Interests, TaxonomyContentItem.FieldName_RelatedInterests);
		}

		if (!_relatedNews.Any() && acrProfile.Modalities != null && acrProfile.Modalities.Count > 0)
		{
			_relatedNews = PerformSearch(acrProfile.Modalities, TaxonomyContentItem.FieldName_RelatedModalities);
		}

		if (!_relatedNews.Any() && acrProfile.Subspecialities != null && acrProfile.Subspecialities.Count > 0)
		{
			_relatedNews = PerformSearch(acrProfile.Subspecialities, TaxonomyContentItem.FieldName_RelatedSubspecialites);
		}
