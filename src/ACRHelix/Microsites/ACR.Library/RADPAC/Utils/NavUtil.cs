...
namespace ACR.Library.Radpac.Utils
{
    public static class NavUtil
    {
        public static Item GetCurrentSection()
            ...
            {
                if (item.ID.ToString() == ItemReference.Radpac.Guid)
                    return ItemReference.Radpac.InnerItem;
                else
            ...
            private static Item CrawlToFirstLevel(Item item)
            {
                ...
                if (item.Parent.ID.ToString() == ItemReference.Radpac.Guid)
                {
                    return item;
                }
                else
            ...
            {
                string templateId =
                    SocialMediaLinkItem.TemplateId;
                IEnumerable<Item> items =
                    parentItem.Axes.GetDescendants().Where(
                       i => i != null
