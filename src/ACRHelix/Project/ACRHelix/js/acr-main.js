...
function squareItUp(elem, _p) {
    return (function (elem, _p) {
        ...
        console.log(elem + " sss " + param.squareItem);
...
function setGoogleAnalyticsSearch(query, gaid, searchUrl) {
    ...
    _gaq.push(['_trackPageview', searchUrl + '?q=' + query]);  

}

function setupSearchGA() {
...
function lookForExternalUrlThenAddIcon(_params) {
    "use strict";
    var initials = {
        elem: 'a',
        outer: 'main',
...
jQuery('.log-out').click(function () { window.location.href = jQuery(this).find('a').first().attr('href') });
...
if (getCookie("hiddenNotificationText") !== jQuery("#dismissible-notification .notification-bar-text").text()) {
...
jQuery('#newsletter-submit-btn').click(function () {
    var email = jQuery('#newsletter-email-tb').val();
    window.location = '/Sign-Up-for-ACR-Newsletters?email=' + encodeURIComponent(email);
    return false;
});*/

//scrollInView('table.has-acr-table', { childItem: 'td', $slideContainer: 'table' });
//scrollInView('.has-acr-table', { childItem: 'td', $slideContainer: 'table' });

$('table.has-acr-table').each(function () {

    var tableParent = $(this).parent();
    //if (tableParent.closest('.acr-tabs-body').closest('.meeting-area').length == 0) {
    //if ($(this).find('table').length > 0) {
    tableParent.scrollInView({ childItem: 'td', $slideContainer: 'table', breakPointsOnWindow: false });
    //}
    //}
    //$('table.has-acr-table').first().closest('.acr-tabs-body').closest('.meeting-area')    
});
...
$('div.has-acr-table').each(function () {
    ...
    //var tableParent = $(this).find('table').parent();
...
jQuery(function ($) {
	    ...
	    $('.tabs').scrollInView({ childItem: 'li', slideWrapper: 'ul', scrollItemAmount: 1 });
	...
	//scrollInView('.tabs', { childItem: 'li', slideWrapper: 'ul', scrollItemAmount: 1 });
	
	/*
    $('.acr-tabs').find('a').click(function () {
        var href = $(this).attr('href').substring(1);
        $('div[data-tab-id="' + href + '"]').find('table.has-acr-table').css('width:100%');
    })*/
