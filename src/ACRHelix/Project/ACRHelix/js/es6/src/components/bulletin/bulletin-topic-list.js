...
class BulletinTopicList {
  constructor(selectID, listAreaID) {
    ...
    if (select.length > 0 && listArea.length > 0) {
      ...
      this.TopicID = listArea.attr("data-topic");
      this.FirstLoad = true;
      this.Init();
    }
  }
  ...
  LoadArticles(year) {
    ...
    jQuery.ajax({
      type: "POST",
      url: "/api/Sitecore/Bulletin/GetTopicArticles",
      datatype: "json",
      data: { topic: this.TopicID, year: year },
      success: data => {
...
export default BulletinTopicList;
